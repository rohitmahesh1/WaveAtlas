...
export default function AdvancedViewerPage(props: { onViewAllRuns?: () => void }) {
  ...
  const downloadVisibleTracks = () => {
    ...
    const rows = filteredTracks.map((track) => ({
      ...
      sample: track.sample ?? "",
      points: track.poly?.length ?? 0,
      measured_peaks: track.metrics?.num_peaks ?? 0,
      peak_candidates: track.metrics?.num_peak_candidates ?? track.peaks?.length ?? 0,
      review_candidates: track.metrics?.num_review_candidates ?? 0,
      ...
      estimated_cycle_count: track.metrics?.estimated_cycle_count ?? "",
      spectral_peak_to_median_ratio: track.metrics?.spectral_peak_to_median_ratio ?? "",
      frequency_agreement_error: track.metrics?.frequency_agreement_error ?? "",
      period_consistency_cv: track.metrics?.period_consistency_cv ?? "",
      peak_prominence_snr: track.metrics?.peak_prominence_snr ?? "",
      measurement_status: track.metrics?.measurement_status ?? "",
      status_reasons: (track.metrics?.status_reasons ?? []).join(";"),
      evidence_rule_version: track.metrics?.evidence_rule_version ?? "",
      frequency_estimator_valid: track.metrics?.frequency_estimator_valid ?? "",
      frequency_valid: track.metrics?.frequency_valid ?? "",
      frequency_method: track.metrics?.frequency_method ?? "",
      frequency_failure_reason: track.metrics?.frequency_failure_reason ?? "",
      frame_sampling_valid: track.metrics?.frame_sampling?.valid ?? "",
      missing_frame_count: track.metrics?.frame_sampling?.missing_frame_count ?? "",
      max_frame_gap: track.metrics?.frame_sampling?.max_frame_gap ?? "",
      frame_coverage_fraction: track.metrics?.frame_sampling?.coverage_fraction ?? "",
      poly: track.poly ?? [],
